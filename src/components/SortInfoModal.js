...
while swapped`,
    ...
    selection: {
      ...
      description: (
        <>
          <Text>
            Starting at index 0, selection sort will perform a linear scan of
            the entire array and find the smallest element amoung all elements.
            Then it will swap that element with the element at index 0. At that
            point we know that the element at index 0 is in its final sorted
            position. The algorithm will continue making such scans at each
            index until all values are in their sorted position.
          </Text>
          <Text>
            ...
            Selection Sort is among the worst performing of the comparison
            sorts. It has few use cases outside of teaching sorting and
      ...
      description: (
        <>
          ...
          <Text>
            ...
            worst case time complexity. However, if the array is nearly sorted
            the algorithmic time complexity trends toward linear time. This
            makes Insertion Sort a potential candidate for sorting if the array
            is nearly sorted. A variation of Insertion Sort and Merge Sort,
            known as Tim Sort, is used in many languages (most notably Python)
            as their primary sort.
          </Text>
        </>
      ),
    ...
    heap: {
      ...
      description: (
        <>
          ...
          <Text>
            Building a heap from an array will take O(n) time, and each swap
            will take O(log(n)) for n values. Therefor, time complexity for Heap
            Sort will be O(n + n*log(n)) which simplifies to O(n*log(n)).
            Although heap sort is a fast sorting algorithm, it generally
            performs worse in practice than its other log-linear time complexity
            sorts, such as Quick Sort and Merge Sort. The underlying heap data
            structure, however, is widely used for maintaining priority queues
            or finding the smallest/largest element of a collection.
          </Text>
        </>
      ),
      ...
      description: (
        <>
          ...
          <Text>
            ...
            sorts. Since Merge Sort is also stable, it is sometimes preferred
            over Quick Sort if the array contains sorted objects rather than
            just integers (when stability matters). However, Merge Sort has a
            linear space complexity due to the auxiliary arrays being created on
            each merge, so if memory is going to be an issue than other sorts
            may serve better.
          </Text>
        </>
      ),
      code: `split each element into partitions of size 1
      ...
      description: (
        <>
          <Text>
            ...
            into smaller portions. On each recursive call, a pivot is selected
            and then each other element is moved to the left or right of the
            pivot depending on its relative size. After each recursive call, the
            pivot that was selected will be in its final position within the
            array.
          </Text>
          <Text>
            Quick Sort can make a series of long range swaps, making it
            unstable. However, it generally out performs all other sorting
            algorithms. Quick Sort has a quadratic worst case time complexity.
            However, the average case time complexity of O(n*log(n)) is
            generally used because the worst case time complexity is nearly
            impossible to get on large input sizes.
          </Text>
        </>
      ),
...
set first element as pivot
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="4xl">
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>{sortInfo[sortType].title}</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <HStack>
            {sortInfo[sortType].badges.map((badge) => (
              <Badge
                variant="solid"
          ...
          <HStack>
            <VStack my="10px">{sortInfo[sortType].description}</VStack>
            <VStack>
              <Code display="block" whiteSpace="pre" mb="10px">
                {sortInfo[sortType].code}
              </Code>
              <StatGroup w="100%" mx="auto" textAlign="center">
                <Stat>
                  <StatLabel>Time Complexity</StatLabel>
                  {sortInfo[sortType].timeComplexity}
                </Stat>
                <Stat>
                  <StatLabel>Space Complexity</StatLabel>
                  {sortInfo[sortType].spaceComplexity}
                </Stat>
              </StatGroup>
            </VStack>
          </HStack>
